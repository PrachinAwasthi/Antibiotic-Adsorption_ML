...
savefig("true data traied_case1.png")
savefig("true data traied_case1.svg")
...
S_testing=Array(sol_testing)
p_trained=opt_sol3.u
prob_nn_testing=ODEProblem(nn_ude!, S_testing[:, 1], testing_tspan, p_trained)

##train the NN
#define predict function
function extended_predict(θ, X=S_testing[:, 1], T=testing_tsteps)
...
nn_ude_pred_sol_testing=extended_predict(p_tested, S_testing[:, 1], testing_tsteps)

combined_pred = vcat(nn_ude_pred_sol[1, :], nn_ude_pred_sol_testing[1, 2:end])
...
savefig("true data_UDE_trained_tested_case1.png")
