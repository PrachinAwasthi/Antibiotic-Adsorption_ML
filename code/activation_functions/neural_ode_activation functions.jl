...
losses_sigmoid= Float32[]
callback= function (p, l, pred; doplot=true)
    push!(losses_sigmoid, l)
    if length(losses_sigmoid)%50==0
    println("Current loss after $(length(losses_sigmoid)) iterations: $(losses_sigmoid[end])")
    end
    return false
...
savefig("true solution_predicted_sigmoid.png")
savefig("true solution_predicted_sigmoid2.svg")
plot(losses_sigmoid, lw=2, seriestype=:line, label="losses_sigmoid", color=:red, title="Losses", xlabel="Number Of Iterations", ylabel="Loss", size=(1200, 600), left_margin=15px, right_margin=15px, bottom_margin=15px, top_margin=15px)
savefig("losses_sigmoid.png")
savefig("losses_sigmoid.svg")
############################################Activation-tanh
#load the packages
using OrdinaryDiffEq, ComponentArrays
using Lux, Flux, DiffEqFlux, Random
...
## define the callback function 
losses_tanh= Float32[]
callback2= function (p, l, pred; doplot=true)
    push!(losses_tanh, l)
    if length(losses_tanh)%50==0
    println("Current loss after $(length(losses_tanh)) iterations: $(losses_tanh[end])")
    end
    return false
...
## solve the optimization problem using optimiser
opt_sol= Optimization.solve(opt_prob, OptimizationOptimisers.Adam(0.001), maxiters=1000, callback=callback2, allow_f_increases=false)

## Again optimizing to converge the predicted solution
opt_prob2=remake(opt_prob, u0 = opt_sol.u)
opt_sol2= Optimization.solve(opt_prob2, OptimizationOptimisers.Adam(0.001), maxiters=10000, callback=callback2, allow_f_increases=false)

#remake the optimization problem
opt_prob3=remake(opt_prob2, u0 = opt_sol2.u)
#solve the problem
opt_sol3=Optimization.solve(opt_prob3, LBFGS(), callback=callback2, maxiters=2000)

nnode_pred_sol= nnode_predict(opt_sol2.u)
...
plot(t, true_sol[1, :], seriestype=:line, lw=5, color=:red, alpha=0.5, label="True Solution", xlabel="Time (min)", ylabel="Adsorption capacity (mg/g)", title="Langmuir Adsorption Model", size=plot_size, left_margin=left_margin, right_margin=right_margin, bottom_margin=bottom_margin, top_margin=top_margin, legend=:bottomright)
plot!(tsteps, nnode_pred_sol[1, :], seriestype=:scatter, marker=:circle, markersize=5.0, lw=3, label="Neural prediction", color=:blue)
savefig("true solution_predicted_tanh.png")
plot(losses_tanh, lw=2, seriestype=:line, label="losses_tanh", color=:red, title="Losses", xlabel="Number Of Iterations", ylabel="Loss", size=(1200, 600), left_margin=15px, right_margin=15px, bottom_margin=15px, top_margin=15px)
savefig("losses_tanh.png")
savefig("losses_tanh.svg")

####plot both the losses into 1 figure
plot(losses_sigmoid, lw=2, seriestype=:line, label="losses_sigmoid", color=:orange, title="Losses", xlabel="Number Of Iterations", ylabel="Loss", size=(1200, 600), left_margin=15px, right_margin=15px, bottom_margin=15px, top_margin=15px)
plot!(losses_tanh, lw=2, seriestype=:line, label="losses_tanh", color=:green)
savefig("losses_sigmoid_tanh.png")
savefig("losses_sigmoid_tanh.svg")