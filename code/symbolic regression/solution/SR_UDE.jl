...
problem = DirectDataDrivenProblem(nn_ude_pred_sol, nn_pred_missing_terms)
println(problem)
@show size(problem)
...
sr_output_file = "SR_UDE_initial.txt"
...
function parameter_loss(p)
    Y = reduce(hcat, map(Base.Fix2(dddsol2_basis, p), eachcol(nn_ude_pred_sol)))
...
# Define the output file path
sr_output_file = "SR_UDE_final.txt"
# Open the file once at the start of the code
open(sr_output_file, "w") do file
...
savefig("01_true data_UDE_reg_pred_data.png")
...
savefig("01_true_UDE_reg_pred_missing_terms.png")